...
export function WhatsAppCta({ className = "", fullWidth = false }: { className?: string; fullWidth?: boolean }) {
  const configured = Boolean(WHATSAPP_URL);
  const classes = `inline-flex min-h-12 items-center justify-center gap-2 rounded-md bg-[#25D366] px-5 py-3 text-sm font-bold tracking-wide text-white shadow-sm transition hover:brightness-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#25D366] focus-visible:ring-offset-2 ${fullWidth ? "w-full" : ""} ${className}`;
  if (!configured) {
